Sachin Nair svn343 Prof Y
Saaketh Vangati srv629 Prof Cuevas
...
;-----------------------My_MOD-----------------------
; Output: R1 MOD R0
; Input: R1 number to be divided and output, R0 number to mod by
; Invariables: This function must not permanently modify registers R4 to R11
My_Mod
	UDIV R2, R0, R1
	MUL R2, R2, R1
...
lr_copy		EQU 16
	...
	MOV R0,#'.'
	BL ST7735_OutChar
	LDR R0,[R11, #thousandths]
	BL LCD_OutDec
	LDR R0,[R11, #hundredths]
	BL LCD_OutDec
	LDR R0,[R11, #tenths]
	BL LCD_OutDec
...
void IO_Init(void) {
 // --UUU-- Code to initialize PF4 and PF2
	...
	GPIO_PORTF_PCTL_R = 0x00000000;   	// 4) PCTL GPIO on PF4 and PF2
	GPIO_PORTF_DIR_R = 0x4;							// 5) PF4 int, PF2 out
	GPIO_PORTF_AFSEL_R = 0x00; 					// 6) disable alt funct on PF7-0
	GPIO_PORTF_PUR_R = 0x14; 						// enable pull-up on PF2 and PF4
