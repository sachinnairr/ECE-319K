...
{
  0,357500,283736,0,238641,0,0,0,212585,0,0,0,0,0,0,0
};
int main(void){       
  DisableInterrupts();
  ...
  Sound_Init();
  Music_Init();
	SYSCTL_RCGCGPIO_R &= ~0x20;		//Clear enable port F clock
	SYSCTL_RCGCGPIO_R |= 0x20;		//Enable port F clock
	__nop();
	__nop();
	__nop();
	__nop();
	GPIO_PORTF_DEN_R |= 0x2;		//Clear enable PF4
	GPIO_PORTF_DIR_R |= 0x2;		//Set PF4
  EnableInterrupts();
	
	uint32_t heartbeat_counter = 0;
	uint32_t lastInput = Key_In();
  while(1)
	{
		if(heartbeat_counter == 0)
		{
			GPIO_PORTF_DATA_R ^= 0x2;	//Write heartbeat
		}
		++heartbeat_counter;
		heartbeat_counter %= 1000000;
    uint32_t currentInput = Key_In();
		if(currentInput != lastInput){
			lastInput = currentInput;
			
			uint32_t period = lookup[currentInput] / 64;
			Sound_Start(period);
		}
  }             
}
...
// Output: none
void Key_Init(void){
	SYSCTL_RCGCGPIO_R &= ~0x10;		//Clear enable port E clock
	...
	GPIO_PORTE_DEN_R &= ~0x0F;		//Clear enable PE0-3
