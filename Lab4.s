;****************** Debug.s ***************
; Program written by: Saaketh Vangati and Sachin Nair
...
Debug_DumpLastValue SPACE 4
Debug_DumpOffset    SPACE 4
	
Debug_ElapsedTimeCounter SPACE 4
Debug_ElapsedTimeLastTime SPACE 4
	
Debug_BeatCounter SPACE 1
...
;you write this
	  ...
	  LDR R0,=Debug_DumpLastValue
	  MOV R1,#-1
	  STR R1,[R0]
	  
	  LDR R0,=Debug_DumpOffset
	  MOV R1,#0
	  STR R1,[R0]
	  ...
	  LDR R0,=Debug_ElapsedTimeCounter
	  MOV R1,#164
	  STR R1,[R0]
		
	  LDR R0,=Debug_BeatCounter
	  MOV R1,#0
	  STR R1,[R0]

	  LDR R0,=SYSCTL_RCGCGPIO_R		;R0 points to SYSCTL_RCGCGPIO_R
	  LDR R1,[R0]					;Read SYSCTL_RCGCGPIO_R into R1
	  ORR R1,#0x20					;Turn on clock Bin: 00100000
	  STR R1,[R0]					;Write back to SYSCTL_RCGCGPIO_R
	  NOP
	  NOP
	  LDR R0,=GPIO_PORTF_DIR_R		;R0 points to GPIO_PORTF_DIR_R
	  MOV R1,#0x2
	  STR R1,[R0]					;PF1 output
	  LDR R0,=GPIO_PORTF_DEN_R		;R0 points to GPIO_PORTF_DEN_R
	  STR R1,[R0]					;PF1 enabled
	  
	  LDR R0,=DumpBuf				;R0 points to DumpBuf
	  ADD R1,R0,#200				;R1 points to end of DumpBuf
	  LDR R2,=0xFFFFFFFF
Debug_Init_Loop
	  CMP R0,R1						;Compare current position to end of DumpBuf
	  BEQ Debug_Init_Leave_Loop		;If at end, jump out of loop
	  STR R2,[R0]
	  ADD R0,#4
	  B Debug_Init_Loop				;Jump back up to loop
Debug_Init_Leave_Loop
...
;Let M = number of instructions in your Debug_Dump = 23 instructions
;Calculate T = M instructions * 2cycles/instruction * 12.5ns/cycle = 23 * 2cycles/instruction * 12.5ns/cycle = 575 ns
;Calculate intrusiveness is T/2.5ms = 575 ns / 2.5 ms = 0.575 ms / 2.5 ms = 23%
	  LDR R0,=Debug_DumpOffset
	  LDR R1,[R0]
	  CMP R1,#164
	  BEQ Debug_Dump_Done
...
Debug_ElapsedTime 
	  ...
	  LDR R3,[R0]
	  CMP R3,#0
	  BEQ Debug_ElapsedTime_Done
	  SUB R3,#-1
	  STR R3,[R0]
	  
	  LDR R0,=TIMER2_TAR_R
	  LDR R1,[R0]							;R1 = TIMER2_TAR_R = NowTime
	  LDR R0,=Debug_ElapsedTimeLastTime
	  LDR R2,[R0]							;R2 = LastTime
	  STR R1,[R0]							;Write NowTime to LastTime location
	  ...
	  SUB R3,R2,R1							;R3 = ElapsedTime
	  
	  LDR R0,=MinimumTime
	  LDR R1,[R0]							;R1 = MinimumTime
	  CMP R3,R1
	  BGE Debug_ElapsedTime_Skip
	  STR R3,[R0]
	  
Debug_ElapsedTime_Skip
	  LDR R0,=MaximumTime
	  LDR R1,[R0]							;R1 = MaximumTime
	  CMP R3,R1
	  BLE Debug_ElapsedTime_Done
	  STR R3,[R0]
	  
Debug_ElapsedTime_Done
      POP {R0-R4,PC}
